...
| 21 | Out-of-Office Automation | ⚠️ | Extracts return date, requeues, shows next send date banner. | **No default-delay fallback when no date** (we skip instead). **Next-send date is the return date itself, not return+1** as the example shows. Written OOO explanation + example email not delivered. |
...
| 23 | Request for Primary Point of Contact | ⚠️ | Category exists (manual) with ONE generic template. | Spec shows **scenario-specific** generated replies (property-mgmt / first-name-only / forwarded / department). AI-suggest intentionally disabled per your earlier instruction — reconcile with §18/§23 which list it as AI-suggested. |
...
| 26 | Missing Lead ID | ⚠️ | Only did a graceful COT fallback draft. | **Missed the pitched workflow**: create a new lead ID, attach the reply, categorize/Send-Reply/COT on it, send from original account, and make a COT-selected email "become the lead." |
