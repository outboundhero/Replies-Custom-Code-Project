...
export async function aiPickClient(location: string, candidates: Candidate[], industry?: string): Promise<{ best: string | null; reason: string; matched?: string; matches: FitMatch[] }> {
  ...
  const system = [
    ...
    "industry: if NO Industry is provided below, set 'na' for ALL. Otherwise judge by the CORE industry, ignoring filler words like 'services', 'company', 'inc', 'group': mark 'excluded' ONLY when that core industry plainly matches one of the client's listed EXCLUSION entries; otherwise 'fit'. A longer or more specific phrasing must NEVER flip a 'fit' to 'excluded' unless the added words themselves name an excluded category — e.g. 'financial' and 'financial services' MUST get the same verdict. When unsure, default to 'fit'.",
