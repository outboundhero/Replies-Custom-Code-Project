...
export async function generatePrimaryContactReply(
  ...
  const systemPrompt = [
    ...
    "{sender_first_name}",
    "",
    "Rules for the ask sentence — these generalize to ANY reply, whatever wording or situation the prospect uses (the examples are illustrative, not an exhaustive list):",
    "  1. Read the prospect's reply and identify whoever they indicate is responsible for the service — an organization, a person, a committee, a department, a family member, a corporate office, anyone.",
    "  2. PREFER the specific organization/entity named in the reply — e.g. 'Highland Hills Parks & Rec handles that' → 'Would you mind sharing the email address of your primary point of contact at Highland Hills Parks & Rec?' Same for an HOA, a franchise corporate office, a school district, a named vendor/management firm, etc.",
    "  3. If the reply names a PERSON or ROLE (even first-name-only or title-only: 'Bob handles this', 'our facilities coordinator decides') → ask for that person's email: 'Would you be able to share Bob's email address?' / '…your facilities coordinator's email address?' then keep the universal reason line.",
    "  4. If they say they FORWARDED/passed it along internally → ask for the email of the person or department they sent it to.",
    "  5. If no responsible party is named in the reply but the provided company/organization data clearly IS the responsible org, you may use it (normalized).",
    "  6. If you are NOT confident who they mean, go GENERIC: 'Would you mind sharing the email address of the primary point of contact who handles that for your location?' NEVER invent or guess a name — a generic ask beats a wrong name.",
    "  7. Phrase the ask naturally ('Would you be able to provide me with…' / 'Would you mind sharing…') — adapt to the reply, do not mechanically insert tokens.",
    "",
    "Other rules:",
